...
public class TEDynamo extends TER implements IEnergyHandler, IFluidHandler{
	...
	public void transmit() {
		for(ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS){
			if(worldObj.getTileEntity(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ) instanceof IEnergyHandler){
				IEnergyHandler conduit = (IEnergyHandler)worldObj.getTileEntity(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ);
	...
	public void readFromNBT(NBTTagCompound comp) {
		...
		NBTTagList list = comp.getTagList("Fluids", 10);

		for(int i = 0; i < list.tagCount(); i++) {
			NBTTagCompound tank = (NBTTagCompound) list.getCompoundTagAt(i);
